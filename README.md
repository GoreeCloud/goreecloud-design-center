...
- Platform Contract `0.2` declaration covering exactly the seven Integral Platform Systems: GoreeCloud Manager, Privacy Shield, Wardveil Security, Everkeep, Glaze UI, GoreeCloud Mesh, and GoreeCloud Identity. GoreeCloud Sync remains separately governed application/service functionality rather than an eighth Platform System.
- Source/repository validation for IDs, internal anchors, `aria-controls`, baseline documentation, runtime dependency boundaries, implemented-versus-required Glaze version planes, and exact seven-system Platform Contract consistency.
...
The GitHub Actions workflow additionally validates `goreecloud.platform.yaml` using the authoritative central Platform Contract `0.2` validator pinned to GoreeCloud/GoreeCloud merge commit `dd7701b116d210a82ec0cb18ccb84b553b6e87e1`.
...
GoreeCloud Sync is evaluated separately when Design Center has an applicable synchronized dataset or cross-device capability. The current stateless/no-account Development runtime does not make Sync an Integral Platform System and does not claim a Sync integration.
